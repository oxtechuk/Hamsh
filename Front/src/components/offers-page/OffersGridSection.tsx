...
function PagBtn({ children, onClick, active = false, disabled = false }: IPagBtnProps) {
  return (
    <button
      type="button"
      onClick={onClick}
        ...
        {totalPages > 1 && (
          <div dir="ltr" className="mt-14 flex items-center justify-center gap-2">
            ...
            {getPageNumbers(currentPage, totalPages).map((p, i) =>
              p === "..." ? (
                <span key={`e-${i}`} className="flex h-[40px] w-[40px] items-center justify-center text-[14px] font-bold text-[#C7232B]">
                  ···
                </span>
              ) : (
                <PagBtn
