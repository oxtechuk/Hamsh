    ...
    >
      ...
      >
        ...
        <p className="text-[14px] font-bold text-[#111111]">
          {t("comparePage.summaryBadge", "الأفضل في المقارنة")}
        </p>
        ...
        <p className="mt-4 text-[16px] font-medium text-[#111111]">
          {isTie ? (
            <>
              {t("comparePage.tieScorePrefix", "تعادلتا في")}{" "}
              <span className="text-[var(--brand-secondary-color)] font-extrabold">
                {car1Score}
              </span>{" "}
              {t("comparePage.scoreSuffix", {
                total: totalComparedCriteria,
                defaultValue: "من {{total}} معيار",
              })}
            </>
          ) : (
            <>
              {t("comparePage.scorePrefix", "تفوقت في")}{" "}
              <span className="text-[var(--brand-secondary-color)] font-extrabold">
                {winnerScore}
              </span>{" "}
              {t("comparePage.scoreSuffix", {
        ...
        {!isTie && (
          ...
          >
            {t("comparePage.getItNow", "احصل عليها الآن")}
          </button>
        )}
      </div>
    </section>
