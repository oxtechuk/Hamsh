...
import { getCities, getCalculatorSettings } from "../../services/api";
...
export default function StepOneForm({ onNext }: IStepOneFormProps) {
  ...
  const otpEnabled = calcSettings?.otp_enabled ?? false;
  const isSaudi = isSaudiPhone(phone);
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (otpEnabled && isSaudi && !isPhoneVerified) {
    ...
    <>
      ...
      >
        <h2 className="text-start text-[25px] font-extrabold leading-tight text-[#111111] sm:text-[29px]">
          {t("financeCalculator.step1.personalInfoTitle", "معلوماتك الشخصية")}
        </h2>

        <form onSubmit={handleSubmit} className="mt-6 space-y-5">
          {/* Full name */}
          <FieldGroup label={t("financeCalculator.step1.fullName", "الاسم الكامل")} required>
            <input
              type="text"
              value={fullName}
              onChange={(event) => setFullName(event.target.value)}
              placeholder={t("financeCalculator.step1.fullNamePlaceholder", "اسمك الكريم")}
              autoComplete="name"
          ...
          {/* Phone */}
          <FieldGroup label={t("financeCalculator.step1.phone", "رقم الجوال")} required>
            <div className="relative">
              <input
                ...
                autoComplete="tel"
                dir="ltr"
                className={`${fieldClassName} text-end ${otpEnabled && isSaudi ? (direction === "rtl" ? "pl-32" : "pr-32") : ""}`}
                required
              />
              ...
              {otpEnabled && isSaudi && (
                ...
                >
                  {isPhoneVerified ? (
                    <span className="inline-flex items-center gap-1.5 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-700 shadow-sm">
                      <CheckCircle2 size={15} />
                      {t("financeCalculator.otp.verifiedBadge", "تم تأكيد الرقم")}
                    </span>
                  ) : (
                    <button
                      type="button"
                      onClick={() => setIsOtpModalOpen(true)}
                      className="inline-flex items-center gap-1.5 rounded-md bg-[#C5232B] px-3.5 py-1.5 text-xs font-bold text-white shadow-sm transition hover:bg-[#A91D24]"
                    >
                      <ShieldCheck size={15} />
                      {t("financeCalculator.otp.verifyButtonShort", "تأكيد الرقم")}
                    </button>
                  )}
                </div>
              )}
          ...
          {/* City */}
          <FieldGroup label={t("financeCalculator.step1.city", "المدينة")} required>
            <div className="relative">
              <select
                value={city}
          ...
          >
            {t("financeCalculator.step1.nextButton", "التالي — اختر السيارة")}
          </button>
        </form>
      </section>
...
function FieldGroup({ label, required = false, children }: IFieldGroupProps) {
  return (
    <div>
      <label className="mb-2 block text-start text-[13px] font-bold text-[#222222]">
        {label}
        {required && <span className="ms-1 text-[#111111]">*</span>}
      </label>
      {children}
    </div>
  );
