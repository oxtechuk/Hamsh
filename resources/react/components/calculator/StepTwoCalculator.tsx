...
function getSliderBg(value: number, min: number, max: number, direction: string) {
  const pct = ((value - min) / (max - min)) * 100;
  const gradientDir = direction === "rtl" ? "to left" : "to right";
  return `linear-gradient(${gradientDir}, var(--brand-secondary-color) ${pct}%, #D1D5DB ${pct}%)`;
}
    ...
    <div dir={direction} className="space-y-4">
      ...
      <div className="rounded-[20px] border border-[#E5E7EB] bg-white px-6 py-6 shadow-sm">
        ...
        <div className="mb-6">
          ...
          <div dir="ltr">
            <input
              ...
              style={{ background: getSliderBg(carPrice, CAR_MIN, CAR_MAX, direction) }}
            />
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="mb-2 flex items-center justify-between text-[13px]">
            <span className="text-[#9CA3AF]">
              {t("financeCalculator.step2.downPayment")} ({downPaymentPercent}%)
            </span>
            <span className="font-bold text-[#111827]">
          ...
          <div dir="ltr">
            <input
              ...
              className="calc-slider"
              style={{ background: getSliderBg(downPaymentPercent, DP_MIN, DP_MAX, direction) }}
            />
          </div>
        </div>
      ...
      <div className="rounded-[20px] bg-[#021F38] px-6 py-6 text-white">
        ...
        <div className="mb-5 grid grid-cols-2 divide-x divide-x-reverse divide-white/10">
          <div className="ps-4 text-center">
            ...
            <p className="mt-1 text-[26px] font-extrabold text-white">
              {totalFinance > 0 ? `${totalFinance.toLocaleString()} ${riyal}` : "\u2014"}
            </p>
          </div>
        ...
        <div className="mb-5 space-y-2 border-t border-white/10 pt-4">
          <div className="flex justify-between text-[13px]">
            <span className="text-white/60">{t("financeCalculator.step2.financeAmount")}</span>
            <span>
              {financeAmount > 0 ? `${financeAmount.toLocaleString()} ${riyal}` : "\u2014"}
            </span>
          </div>
          <div className="flex justify-between text-[13px]">
            <span className="text-white/60">{t("financeCalculator.step2.totalInterest")}</span>
            <span>
          ...
          <div className="flex justify-between text-[13px]">
            ...
            <span>
              {calcResult?.annual_rate ? `${calcResult.annual_rate}%` : "\u2014"}
            </span>
          </div>
        </div>
