...
@section('title', __('تعديل سيارة') . ' | hamsh ')
...
@section('content')
    <div class="container-fluid" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
        ...
        <form action="{{ route('crm.cars.update', $car) }}" method="POST" enctype="multipart/form-data">
            ...
            @if($errors->any())
                <div class="alert alert-danger mb-4"><i
                        class="bi bi-exclamation-triangle-fill me-2"></i>{{ __('يوجد أخطاء في البيانات') }}</div>
            @endif
            ...
            <div class="row g-4">
                <div class="col-12 col-lg-8">
                    ...
                    <div class="car-section mb-4">
                        <div class="car-section-header"><i class="bi bi-info-circle"></i> {{ __('البيانات الأساسية') }}
                        </div>
                                                ...
                                                <div id="models_items_container">
                                                    @foreach($existingModels as $m)
                                                        <li class="model-item-wrapper" data-brand-id="{{ $m->brand_id }}">
                                                            <a class="dropdown-item rounded-2 py-1.5 px-2 model-option-item {{ $car->model === $m->model ? 'active' : '' }}" href="javascript:void(0)" onclick="selectModel('{{ $m->model }}')">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <textarea name="description[ar]" class="form-control" rows="3"
                                        dir="rtl">{{ $car->getTranslation('description', 'ar', false) }}</textarea>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Description (English)</label>
                                    <textarea name="description[en]" class="form-control" rows="3"
                                        dir="ltr">{{ $car->getTranslation('description', 'en', false) }}</textarea>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">{{ __('المميزات (عربي)') }}</label>
                                    <textarea name="features[ar]" class="form-control" rows="3"
                                        dir="rtl">{{ $car->getTranslation('features', 'ar', false) }}</textarea>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Features (English)</label>
                                    <textarea name="features[en]" class="form-control" rows="3"
                                        dir="ltr">{{ $car->getTranslation('features', 'en', false) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- التسعير --}}
                    <div class="car-section mb-4">
                        <div class="car-section-header"><i class="bi bi-currency-dollar"></i> {{ __('التسعير والتقسيط') }}
                        </div>
                        <div class="car-section-body">
                            <div class="row g-3">
                                <div class="col-md-4">
                                    ...
                                    <div class="input-group">
                                        <input type="number" name="cash_price" class="form-control"
                                            value="{{ $car->cash_price }}" min="0" required>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="input-group">
                                        <input type="number" name="min_down_payment" class="form-control"
                                            value="{{ $car->min_down_payment }}" min="0" required>
                                        <span class="input-group-text">{!! __('ريال') !!}</span>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <div class="input-group">
                                        <input type="number" name="min_installment" class="form-control"
                                            value="{{ $car->min_installment }}" min="0" required>
                                        <span class="input-group-text">{!! __('ريال') !!}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="car-section-body">
                            <div class="row g-3 mb-4">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="text" name="specs[fuel]" class="form-control" placeholder="بنزين"
                                        value="{{ $car->specs['fuel'] ?? '' }}">
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label">{{ __('ناقل الحركة') }}</label>
                                    <input type="text" name="specs[gearbox]" class="form-control" placeholder="أوتوماتيك"
                                        value="{{ $car->specs['gearbox'] ?? '' }}">
                                </div>
                                <div class="col-md-3">
                    ...
                    <div class="car-section mb-4">
                        ...
                        <div class="car-section-body">
                            ...
                            <p id="no-colors-msg" class="text-muted text-center py-3"
                                style="font-size:13px;{{ $car->colors && count($car->colors) > 0 ? 'display:none;' : '' }}">
                        ...
                        <div class="car-section-body">
                            ...
                            <div class="d-flex flex-column p-3 rounded-3 mb-3"
                                ...
                                <select name="is_highlighted" class="form-select form-select-sm">
                                    <option value="none" {{ $car->is_highlighted == 'none' ? 'selected' : '' }}>
                                        {{ __('بدون تمييز') }}</option>
                                    <option value="new_arrival" {{ $car->is_highlighted == 'new_arrival' ? 'selected' : '' }}>
                                        {{ __('أحدث السيارات') }}</option>
                                    <option value="featured" {{ $car->is_highlighted == 'featured' ? 'selected' : '' }}>
                                        {{ __('سيارات مختارة') }}</option>
                                    <option value="trending" {{ $car->is_highlighted == 'trending' ? 'selected' : '' }}>
                                        {{ __('الأكثر طلباً') }}</option>
                                    <option value="exclusive" {{ $car->is_highlighted == 'exclusive' ? 'selected' : '' }}>
                                        {{ __('إصدار خاص') }}</option>
                                </select>
                            </div>
                        ...
                        <div class="car-section-body">
                            <div class="row g-3">
                                <div class="col-12">
                                    ...
                                    @if($car->thumbnail)
                                        <div class="d-flex align-items-center gap-3 mb-2 p-2 rounded-3"
                                            style="background:#F8F9FC;border:1px solid var(--crm-border);">
                                        ...
                                        <div class="d-flex flex-wrap gap-2 mt-2">
                                            @foreach($car->images->where('type', 'exterior') as $img)
                                                <div class="position-relative">
                                                    <img src="{{ $img->image_path }}" class="rounded"
                    ...
                    <div class="car-section">
                        <div class="car-section-body">
                            ...
                            <a href="{{ route('crm.cars.index') }}" class="btn-crm-light w-100 mt-2"
                                style="justify-content:center;">{{ __('إلغاء') }}</a>
                        </div>
                    </div>
                </div>
            </div>
...
@section('scripts')
    ...
    <script>
        ...
        // Initialize model list on page load according to selected brand
        document.addEventListener('DOMContentLoaded', function() {
            const initialBrand = document.getElementById('car_brand_select')?.value;
            if (initialBrand) {
                filterModelsByBrand(initialBrand);
            }
        });
