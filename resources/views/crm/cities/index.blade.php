...
@section('title', __('المدن') . ' | AutoCRM')

@section('content')
    <div class="container-fluid" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                <h4 class="mb-1 fw-bold">{{ __('المدن') }}</h4>
                <p class="text-muted mb-0 small">{{ __('المدن المتاحة للحجز من المتجر') }}</p>
            </div>
            <div class="d-flex gap-2">
                @can('manage-cities')
                <button class="btn btn-primary btn-sm rounded-pill px-3 shadow-sm" data-bs-toggle="modal" data-bs-target="#addCityModal">
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @forelse ($cities as $city)
                            <tr>
                                <td class="px-4 text-muted small">{{ $city->id }}</td>
                                <td class="fw-bold text-dark">{{ $city->name }}</td>
                                <td class="text-muted">{{ $city->sort_order }}</td>
                                ...
                                <td class="px-4 text-end">
                                    <div class="d-flex gap-1 justify-content-end">
                                        @can('manage-cities')
                                        <button class="btn btn-sm btn-white border shadow-xs rounded-2" data-bs-toggle="modal" data-bs-target="#editModal{{ $city->id }}">
                                            <i class="bi bi-pencil text-muted"></i>
                                        </button>
                                        @endcan
                                        @can('manage-cities')
                                        <form action="{{ route('crm.cities.destroy', $city) }}" method="POST" onsubmit="return confirm('{{ __('تأكيد الحذف؟') }}')">
                                            @csrf @method('DELETE')
                            ...
                            <div class="modal fade" id="editModal{{ $city->id }}" tabindex="-1">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content border-0 shadow rounded-4 overflow-hidden">
                                        ...
                                        <form action="{{ route('crm.cities.update', $city) }}" method="POST">
                                            @csrf @method('PUT')
                                            <div class="modal-body p-4 pt-2">
                                                <div class="mb-3">
                                                    <label class="form-label fw-bold small text-muted">{{ __('الاسم') }}</label>
                                                    <input type="text" name="name" class="form-control bg-light border-0 shadow-none" value="{{ $city->name }}" required>
                                                </div>
                                                <div class="mb-3">
                                                    <label class="form-label fw-bold small text-muted">{{ __('ترتيب') }}</label>
                                                    <input type="number" name="sort_order" class="form-control bg-light border-0 shadow-none" value="{{ $city->sort_order }}">
                                                </div>
                                                <div class="form-check form-switch p-3 bg-light rounded-3">
                                                    <input class="form-check-input {{ app()->getLocale() == 'ar' ? 'ms-0 me-2 float-none' : '' }}" type="checkbox" name="is_active" value="1" id="act{{ $city->id }}" {{ $city->is_active ? 'checked' : '' }}>
                                                    <label class="form-check-label fw-bold ms-2" for="act{{ $city->id }}">{{ __('نشط') }}</label>
                                                </div>
                                            </div>
        ...
        <div class="modal fade" id="addCityModal" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow rounded-4 overflow-hidden">
                    <div class="modal-header border-0 pt-4 px-4">
                        <h5 class="modal-title fw-bold">{{ __('مدينة جديدة') }}</h5>
                        <button type="button" class="btn-close {{ app()->getLocale() == 'ar' ? 'ms-0 me-auto' : '' }}" data-bs-dismiss="modal"></button>
                    </div>
