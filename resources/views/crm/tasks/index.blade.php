...
@section('title', __('المهام') . ' | hamsh  CRM')
    ...
    <div class="container-fluid" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
        ...
        <div class="d-flex gap-2 mb-4 flex-wrap">
            <a href="{{ route('crm.tasks.index') }}"
                class="btn-crm-{{ !$status ? 'primary' : 'light' }}">{{ __('الكل') }}</a>
            <a href="{{ route('crm.tasks.index') }}?status=new"
                class="btn-crm-{{ $status == 'new' ? 'primary' : 'light' }}">{{ __('جديدة') }}</a>
            <a href="{{ route('crm.tasks.index') }}?status=in_progress"
                class="btn-crm-{{ $status == 'in_progress' ? 'primary' : 'light' }}">{{ __('قيد التنفيذ') }}</a>
            <a href="{{ route('crm.tasks.index') }}?status=done"
                class="btn-crm-{{ $status == 'done' ? 'primary' : 'light' }}">{{ __('مكتملة') }}</a>
        </div>
            ...
            @forelse($tasks as $task)
                <div class="col-12 col-md-6 col-xl-4">
                    <div class="crm-card h-100"
                        style="position:relative; border-right: 4px solid {{ $task->priority == 'high' ? 'var(--crm-red)' : ($task->priority == 'medium' ? '#FF9800' : '#4CAF50') }}">
                        {{-- Priority Badge --}}
                        <div class="d-flex align-items-start justify-content-between mb-12">
                            <span
                                class="badge-{{ $task->priority == 'high' ? 'rejected' : ($task->priority == 'medium' ? 'pending' : 'active') }}">
                                {{ $task->priority_label }}
                            </span>
                            <span
                                class="badge-{{ $task->status == 'done' ? 'done' : ($task->status == 'in_progress' ? 'pending' : 'new') }}">
                                {{ $task->status_label }}
                            </span>
                        </div>

                        <h6
                            style="font-weight:800;font-size:15px;color:#1a1a2e;margin-bottom:8px;{{ $task->status == 'done' ? 'text-decoration:line-through;opacity:0.5;' : '' }}">
                            {{ $task->title }}
                        </h6>
                        ...
                        <div class="d-flex align-items-center gap-2 mt-auto"
                            style="font-size:12px;color:#aaa;border-top:1px solid #f5f5f5;padding-top:12px;margin-top:12px;">
                            @if($task->assignedTo)
                            ...
                            @if($task->due_date)
                                ...
                                <span
                                    style="{{ $task->due_date->isPast() && $task->status != 'done' ? 'color:var(--crm-red);font-weight:700;' : '' }}">
                            ...
                            <div class="d-flex gap-1 me-auto">
                                @can('manage-tasks')
                                    <form action="{{ route('crm.tasks.toggle', $task) }}" method="POST">
                                        @csrf @method('PATCH')
                                        <button class="btn-crm-light" style="padding:5px 10px;font-size:11px;"
                                ...
                                @can('manage-tasks')
                                    <form action="{{ route('crm.tasks.destroy', $task) }}" method="POST"
                                        onsubmit="return confirm('{{ __('حذف هذه المهمة؟') }}')">
                                        @csrf @method('DELETE')
                                        <button class="btn-crm-light" style="padding:5px 10px;font-size:11px;color:var(--crm-red);"
                                            title="{{ __('حذف') }}">
                ...
                <div class="modal-content border-0 rounded-4" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
                    ...
                    <form action="{{ route('crm.tasks.store') }}" method="POST">
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label fw-bold">{{ __('عنوان المهمة') }} <span
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description" class="form-control rounded-3" rows="3"
                                    placeholder="{{ __('تفاصيل المهمة...') }}"></textarea>
                            </div>
                            <div class="row g-3">
                                <div class="col-md-4">
