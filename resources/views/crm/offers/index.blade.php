        ...
        <div class="col-12 col-md-6 col-xl-4">
            <div class="card border-0 shadow-sm h-100 rounded-4 overflow-hidden position-relative">
                ...
                <div class="card-body p-4 ps-5">
                            ...
                            <div class="d-flex align-items-center gap-2 flex-wrap">
                                ...
                                @if($offer->offer_category)
                                    <span class="badge bg-info-subtle text-info small">
                                        <i class="bi bi-tag me-1"></i>{{ $offer->offer_category }}
                                    </span>
                                @endif
                                @if($offer->user_rated_count > 0)
                                    <span class="badge bg-warning-subtle text-warning small">
                                        <i class="bi bi-star me-1"></i>{{ $offer->user_rated_count }} {{ __('تقييم') }}
                                    </span>
                                @endif
                                @if(!$offer->is_active)
                    ...
                    <div class="mt-2 countdown-bar d-flex gap-1 justify-content-center" data-ends-at="{{ $offer->ends_at->toIso8601String() }}">
                        <span class="badge bg-primary-subtle text-primary fw-bold countdown-days">--</span>
                        <span class="badge bg-primary-subtle text-primary fw-bold countdown-hours">--</span>
                        <span class="badge bg-primary-subtle text-primary fw-bold countdown-minutes">--</span>
                        <span class="badge bg-primary-subtle text-primary fw-bold countdown-seconds">--</span>
                    </div>
                    @endif
                </div>
...
<style>
    ...
    .countdown-bar .badge { font-size: 11px; padding: 3px 6px; min-width: 38px; }
</style>
<script>
document.addEventListener('DOMContentLoaded', function () {
    function updateCountdowns() {
        document.querySelectorAll('.countdown-bar').forEach(function (el) {
            const endsAt = new Date(el.dataset.endsAt);
            const now = new Date();
            const diff = endsAt - now;
            if (diff <= 0) {
                el.querySelector('.countdown-days').textContent = '0 {{ __('يوم') }}';
                el.querySelector('.countdown-hours').textContent = '0 {{ __('ساعة') }}';
                el.querySelector('.countdown-minutes').textContent = '0 {{ __('دقيقة') }}';
                el.querySelector('.countdown-seconds').textContent = '0 {{ __('ثانية') }}';
                el.querySelectorAll('.badge').forEach(b => b.classList.replace('bg-primary-subtle', 'bg-danger-subtle'));
                el.querySelectorAll('.badge').forEach(b => b.classList.replace('text-primary', 'text-danger'));
                return;
            }
            ...
            el.querySelector('.countdown-days').textContent = days + ' {{ __('يوم') }}';
            el.querySelector('.countdown-hours').textContent = hours + ' {{ __('ساعة') }}';
            el.querySelector('.countdown-minutes').textContent = minutes + ' {{ __('دقيقة') }}';
            el.querySelector('.countdown-seconds').textContent = seconds + ' {{ __('ثانية') }}';
        });
    }
    updateCountdowns();
