...
<head>
    ...
    <title>{{ __('ممنوع الدخول') }} — GR Motors</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .slash-icon {
            ...
            animation: slashSway 2s ease-in-out infinite;
        }
        @keyframes slashSway {
...
<body>
    ...
    <div class="err-wrap">
        <div class="icon-wrap">
            <i class="bi bi-slash-circle-fill slash-icon"></i>
        </div>
        <div class="error-code">403</div>
        <div class="err-divider"></div>
        <h1 class="error-title">{{ __('عذراً! الوصول إلى هذه الصفحة') }} <span>{{ __('ممنوع') }}</span></h1>
        <p class="error-subtitle">{{ __('ليست لديك الصلاحيات اللازمة للوصول إلى هذه الصفحة. إذا كنت تعتقد أن هذا خطأ، يرجى التواصل مع الإدارة.') }}</p>
        <div class="err-btn-group">
            <a href="{{ route('store.home') }}" class="btn-err btn-err--primary">
                <i class="bi bi-house-door-fill"></i>
                {{ __('الصفحة الرئيسية') }}
            </a>
            <a href="/about" class="btn-err btn-err--ghost">
