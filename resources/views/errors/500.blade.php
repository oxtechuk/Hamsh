...
<head>
    ...
    <title>{{ __('خطأ في الخادم') }} — GR Motors</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .gear-icon {
            animation: gearWarn 1.5s ease-in-out infinite;
        }
        @keyframes gearWarn {
...
<body>
    ...
    <div class="err-wrap">
        ...
        <h1 class="error-title">{{ __('عذراً! حدث') }} <span>{{ __('خطأ في الخادم') }}</span></h1>
        <p class="error-subtitle">{{ __('نواجه بعض المشاكل التقنية حالياً. فريقنا يعمل على إصلاحها في أقرب وقت. يرجى المحاولة لاحقاً.') }}</p>
        <div class="err-btn-group">
            <a href="{{ route('store.home') }}" class="btn-err btn-err--primary">
                <i class="bi bi-house-door-fill"></i>
                {{ __('العودة للرئيسية') }}
            </a>
            <a href="/cars" class="btn-err btn-err--ghost">
