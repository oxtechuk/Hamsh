...
<head>
    ...
    <title>{{ __('طلبات كثيرة جداً') }} — Nawader </title>
...
<body>
    ...
    <div class="err-wrap">
        <div class="icon-wrap">
            <i class="bi bi-sign-stop-fill signal-icon"></i>
        </div>
        <div class="error-code">429</div>
        <div class="err-divider"></div>
        <h1 class="error-title">{{ __('طلبات كثيرة جداً') }} <span>{{ __('حاول لاحقاً') }}</span></h1>
        <p class="error-subtitle">{{ __('لقد تلقينا عدداً كبيراً جداً من الطلبات من جهازك. يرجى الانتظار قليلاً قبل المحاولة مرة أخرى.') }}</p>
        <div class="err-btn-group">
            <a href="{{ url()->previous() }}" class="btn-err btn-err--primary">
                <i class="bi bi-arrow-repeat"></i>
                {{ __('المحاولة مرة أخرى') }}
            </a>
